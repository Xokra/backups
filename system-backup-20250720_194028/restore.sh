...
install_packages() {
    ...
    while IFS= read -r package; do
        ...
        ((current++))
        echo -ne "\r${BLUE}[INFO]${NC} Installing package $current/$package_count: $package"
        ...
        if [[ "$manager" == "universal" ]]; then
            local translated_packages=$(translate_package "$package" "$platform")
            [[ -z "$translated_packages" ]] && continue  # Skip if empty (like fontconfig on macOS)

            
            # Install translated packages based on platform
            case $platform in

                "arch") 
                    for pkg in $translated_packages; do
                        sudo pacman -S --noconfirm "$pkg" >/dev/null 2>&1 && install_success=true && break
                        command -v yay >/dev/null && yay -S --noconfirm "$pkg" >/dev/null 2>&1 && install_success=true && break
                    done ;;
                "mac") 
                    for pkg in $translated_packages; do
                        brew install "$pkg" >/dev/null 2>&1 && install_success=true && break
                    done ;;
                "wsl") 

                    for pkg in $translated_packages; do
                        sudo apt install -y $pkg >/dev/null 2>&1 && install_success=true && break
                    done ;;
            esac
        else
            ...
            case $manager in
                ...
                "aur") 
                    command -v yay >/dev/null && yay -S --noconfirm "$package" >/dev/null 2>&1 && install_success=true ||
                    command -v paru >/dev/null && paru -S --noconfirm "$package" >/dev/null 2>&1 && install_success=true ;;
                "platform-specific")
                    case $platform in
                        "arch") sudo pacman -S --noconfirm "$package" >/dev/null 2>&1 && install_success=true ;;
    ...
    if [[ ${#failed_packages[@]} -gt 0 ]]; then
        ...
        log_success "✅ All $manager packages installed successfully"
    fi
}
