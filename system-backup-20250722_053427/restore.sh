...
is_package_installed() {
    ...
    case "$manager-$platform" in
        "curated-arch"|"dotfile-deps-arch") pacman -Qi "$package" >/dev/null 2>&1 ;;
        "curated-mac"|"dotfile-deps-mac") brew list "$package" >/dev/null 2>&1 ;;
        "curated-wsl"|"dotfile-deps-wsl") dpkg -s "$package" >/dev/null 2>&1 ;;

        "cargo-"*) command -v "$package" >/dev/null 2>&1 ;;
        "pip-"*) pip3 show "$package" >/dev/null 2>&1 ;;
        "npm-"*) npm list -g "$package" >/dev/null 2>&1 ;;
        "brew-"*) brew list "$package" >/dev/null 2>&1 ;;
        "cask-"*) brew list --cask "$package" >/dev/null 2>&1 ;;
        "mas-"*) mas list | grep -q "$package" ;;
...
# FIXED: Merge and deduplicate packages before installation
prepare_packages() {
    log_info "📋 Preparing and deduplicating package lists..."
    
    # Merge system packages (curated + dotfile-deps)
    if [[ -f "config/packages.curated" ]] || [[ -f "config/packages.dotfile-deps" ]]; then

        cat config/packages.curated config/packages.dotfile-deps 2>/dev/null | grep -v '^#' | grep -v '^[[:space:]]*$' | sort -u > config/packages.system

        local system_count=$(wc -l < "config/packages.system" 2>/dev/null || echo 0)
        log_info "📦 Merged ${system_count} unique system packages"

    else
        touch config/packages.system
    fi
}
...
install_packages() {
    ...
    while IFS= read -r package; do
        ...
        if is_package_installed "$platform" "$manager" "$package"; then
    ...
    if [[ ${#skipped_packages[@]} -gt 0 ]]; then
        log_info "Skipped ${#skipped_packages[@]} already installed $manager packages"
    fi
...
main() {
    ...
    # FIXED: Prepare and deduplicate packages
    prepare_packages
    ...
    # FIXED: Install packages in proper dependency order
    log_info "🔧 Installing packages..."
    for manager in system brew cask cargo pip npm aur mas; do
