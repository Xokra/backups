...
# Cross-platform package name translation (only for different names)
translate_package() {
    local platform=$1 package=$2
    
    case "$platform:$package" in
        # Python pip - different names
        "wsl:python-pip") echo "python3-pip" ;;
        "arch:python-pip") echo "python-pip" ;;  
        "mac:python-pip") echo "python" ;;
        
        # Node.js - different names  

        "mac:nodejs") echo "node" ;;
        "wsl:nodejs"|"arch:nodejs") echo "nodejs" ;;
        
        # NPM - usually bundled differently
        "mac:npm") echo "" ;; # bundled with node on mac
        

        # Default: return original if same across platforms
        *) echo "$package" ;;
    esac
}
...
install_packages() {
    ...
    setup_package_manager "$platform" "$manager"
    ...
    while IFS= read -r package; do
        ...
        [[ -z "$translated_package" ]] && continue # Skip if empty (like npm on mac)

        
        local install_success=false
        case "$platform:$manager" in
            # Platform-specific system packages
            "arch:essential"|"arch:desktop"|"arch:dotfile-deps")
                sudo pacman -S --noconfirm "$translated_package" 2>/dev/null && install_success=true ||
            ...
            "mac:essential"|"mac:desktop"|"mac:dotfile-deps")
                brew install "$translated_package" 2>/dev/null && install_success=true ;;
                
            "wsl:essential"|"wsl:desktop"|"wsl:dotfile-deps")
            ...
            *:npm) [[ "$translated_package" != "lib" ]] && npm install -g "$translated_package" 2>/dev/null && install_success=true ;;
...
setup_package_manager() {
    ...
    case "$platform:$manager" in
        ...
        "mac:mas") 
            command -v mas >/dev/null || brew install mas || { log_error "Failed to install mas"; return 1; } ;;
        *:cargo) 
...
main() {
    ...
    for manager in essential desktop dotfile-deps cargo pip npm brew cask mas aur; do
