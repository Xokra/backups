...
configure_zsh() {
    ...
    # Change default shell if not already zsh - FIXED: Use sudo chsh
...
# Package name translation for cross-platform compatibility
translate_package_name() {
    local platform=$1 manager=$2 package=$3
    
    # Platform-specific package name translations
    case "$platform-$manager-$package" in

        # Python pip package name differences
        "arch-curated-python3-pip") echo "python-pip" ;;
        "wsl-curated-python-pip") echo "python3-pip" ;;
        # Node.js package differences  
        "arch-curated-nodejs") echo "nodejs npm" ;;

        # Default: return original package name
        *) echo "$package" ;;
    esac
}
        ...
        case $manager in
            "curated")
                case $platform in
                    ...
                    "arch") 
                        # Handle multiple packages (like "nodejs npm")
